...
fit_min, fit_max = 0.10, 0.16  # fit window
nsigma = 3.0  # for significance computation
pt_min_vals = np.arange(0., 1.1 + 1e-6, 0.05)
S4S9_low_min_vals = np.arange(0.85, 0.89 + 1e-6, 0.005)
S4S9_high_min_vals = np.arange(0.92, 0.96 + 1e-6, 0.005)

REGIONS = {
  "EE_low": {
    "eta_expr": "abs_etaPi0_cor > 1.479 && abs_etaPi0_cor < 1.8",
    "ptG1_min": 0.75,
    "ptG2_min": 0.4,
    "ptPi0_min": 3.75,
    "S4S9_1_min": 0.85,
  ...
  "EE_high": {
    "eta_expr": "abs_etaPi0_cor > 1.8 && abs_etaPi0_cor < 2.4",
    "ptG1_min": 0.95,
    "ptG2_min": 0.95,
    "ptPi0_min": 2.0,
    "S4S9_1_min": 0.92,
...
def save_hist_postfit(h, func, outdir, fout, plot_name=""):
    ...
    func.Draw("same")
    # fit results
    if "EE_low" in plot_name:
        pave = ROOT.TPaveText(0.6, 0.20, 0.9, 0.50, "NDC")
    else:
        pave = ROOT.TPaveText(0.6, 0.15, 0.9, 0.45, "NDC")
    pave.SetFillColor(0)
...
def scan2D_ptG1G2(name, df_base, outdir, fout):
    x0 = pt_min_vals[0] - 0.025
    x1 = pt_min_vals[-1] + 0.025
    nb = len(pt_min_vals)
...
def scan2D_S4S9(name, df_base, outdir, fout):
    if name == "EE_low":
        x0 = S4S9_low_min_vals[0] - 0.0025
        x1 = S4S9_low_min_vals[-1] + 0.0025
        nb = len(S4S9_low_min_vals)
    else:
        x0 = S4S9_high_min_vals[0] - 0.0025
        x1 = S4S9_high_min_vals[-1] + 0.0025
...
def main():
    ...
    fout = ROOT.TFile(f"{outdir}EE_histograms.root", "RECREATE")
    csv_path = outdir + "EE_best_results.csv"
